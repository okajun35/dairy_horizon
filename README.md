...
牛舎の現在不足と将来の暑熱期間を整理し、「第1期」と「全数整備」を比較する暑熱対策の適応投資経路ナビゲーターです。
...
ブラウザで `http://127.0.0.1:8000/` を開くと、主旨と千葉市の期間別THI背景を確認できます。「自分の牛舎で確認する」から `/check` へ進みます。確認画面の初期条件は千葉市・60頭・2列・既存10台です。
...
- 確認画面では会話形式で入力候補を確認し、AI読み解き後は実ファン台数または運転時間を一問だけ補完
...
- 計算済みの牛舎・THI・財務結果だけをOpenAI APIへ送り、条件と次の確認事項を利用者操作時に説明。回答後はPythonで再計算し、失敗時は定型文へフォールバック
