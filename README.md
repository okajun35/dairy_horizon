...
ブラウザで `http://127.0.0.1:8000/` を開くと、主旨と千葉市の期間別THI背景を確認できます。「60頭から5年後45頭のデモを見る」から主デモへ、「自分の牛舎で確認する」から将来頭数未入力の `/check` へ進みます。確認画面の初期条件は千葉市・60頭・2列・既存10台です。
...
- 牛体付近風速で条件を満たした対象頭数を入力すると、追加費用を変えずに回収計算の対象頭数を置き換えて再計算
...
- 決定論的コードが「将来頭数→牛体付近風速→夏季乳量差」の順で次の未確認事項を選び、OpenAI APIは計算済み結果と選択理由を説明。回答後はPythonで再計算し、失敗時は定型文へフォールバック
