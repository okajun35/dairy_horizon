...
- 保存済みCMIP6複数モデルから、2026〜2030年と2031〜2034年のTHI対象日数を年平均の中央値・モデル範囲で表示
- 気象庁・千葉の2020〜2025年日別観測と、同期間のCMIP6モデル基準を事前生成。現在基準との比較画面への接続は次段階
- THI対象日数と明示した運転時間仮定から、ファン台数を変えずに各追加案の年間電力費レンジを表示
- 計算済みの牛舎・THI・財務結果だけをOpenAI APIへ送り、条件と次の確認事項を利用者操作時に説明。失敗時は定型文へフォールバック
