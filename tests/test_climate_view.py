...
class ClimateBackgroundViewTest(unittest.TestCase):
    def test_saved_thi_periods_are_separate_from_fan_counts(self) -> None:
        ...
        self.assertEqual(near_future["median_annual_days"], 96.2)
        self.assertEqual(near_future["minimum_annual_days"], 81.6)
        self.assertEqual(near_future["maximum_annual_days"], 102.6)

        first_phase_cost = near_future["plans"][0]
        self.assertEqual(first_phase_cost["additional_fan_count"], 5)
        self.assertEqual(first_phase_cost["annual_electricity_median_yen"], 124706)
        self.assertEqual(first_phase_cost["annual_electricity_minimum_yen"], 110515)
        self.assertEqual(first_phase_cost["annual_electricity_maximum_yen"], 130927)
    ...
    def test_screen_explains_thi_uncertainty_and_cost_connection(self) -> None:
        ...
        self.assertIn("96日／年", response.text)
        self.assertIn("モデル範囲</dt><dd>82〜103日／年", response.text)
        self.assertIn("2031〜2034年", response.text)
        self.assertIn("98日／年", response.text)
        self.assertIn("モデル範囲</dt><dd>87〜103日／年", response.text)
        ...
        self.assertIn("中央値 124,706円／年", response.text)
        self.assertIn("範囲 110,515円〜130,927円／年", response.text)
    ...
    def test_climate_costs_follow_user_plan_without_changing_it(self) -> None:
        ...
        self.assertIn("中央値 74,824円／年", climate_section.group(0))
